...
| Current shipping version | `1.5 (build 15)` (Sep 2026) — Wendler 5/3/1 + BBB on a per-exercise progression rule with training maxes, Progress tab scoped to one program, template ordering fix. Build 14 (Sep 2026) added straight-set strength + the honest program catalog, inline PR detection and crash-recovery fixes |
...
- **Progression rule** — `workout_day_exercises.progression_rule` ∈ `{autoregulated, wave_531}` (Sep 2026, `20260914_progression_rule_and_training_max.sql`). *Who decides the numbers.* `autoregulated` (default) is the engine; `wave_531` is Wendler's 5/3/1. Three orthogonal axes: `training_mode` (rep band / effort), `set_scheme` (layout), `progression_rule` (authority). A non-autoregulated rule owns the layout, so `set_scheme`, `target_sets` and `rep_cap` are ignored under it. Strength-only; switching the mode drops it. A program rule is added **only when a program's identity is the rule and the engine can't express it** — the catalog cut of Sep 2026 stands otherwise.
- **Training max (TM)** — `training_maxes` table, per user + exercise, **append-only** (latest row wins; `source` ∈ `manual | estimated | bump | hold | reset | recalibrated`). 5/3/1 sets it at 90% of a true or estimated 1RM so every prescribed set is submaximal. Asked once, in the logger, the first time a wave lift is logged (`TrainingMaxCard`, prefilled from best e1RM × 0.9 when history exists); nothing on that lift can be logged until it's answered. Moves only at the cycle boundary.
- **Wave** — one 5/3/1 session: three sets at `TM × {65/75/85 | 70/80/90 | 75/85/95 | 40/50/60}%` for the 5s / 3s / 5/3/1 / deload weeks, the last set of a non-deload wave an **AMRAP** (`5+`, `3+`, `1+`; the stored target is the *minimum*, so adherence stays binary). Wave index is carried in `progression_log.mesocycle_week` (`ProgressionTarget.programWeek`) as "the wave the *next* session runs" — derived from the journal, not a session count, so backdated workouts can't skew it. Rounding is to the *nearest* increment (`WaveProgression.round`), unlike the engine's round-down.
- **Progression target** — output of `ProgressionService.calculateTarget` (or `WaveProgression.nextTarget` for a wave lift): prescribed `targetWeight × targetReps @ targetRPE` for next session, plus a `decision` ∈ `{increaseWeight, increaseReps, maintain, deload, deloadVolume, wave}` and English `reasoning`. `.wave` is **neutral** — a program moved to its next scheduled step; `fetchProgressionRate` skips it and reads the lift's last real verdict (reaching past the 28-day window if the cycle hasn't closed inside it).
...
- Tables of note (still live): `workout_sessions`, `workout_sets`, `exercises`, `templates`, `workout_days`, `workout_day_exercises`, `profiles`, `personal_records`, `goals`, `training_maxes` (Sep 2026), and the monthly-digest tables. `progression_log.mesocycle_week`, unused since the initial schema, now carries a wave lift's next wave index.
...
- Written on every set completion, on a 30s tick (`autoSaveTask`), and when the scene goes inactive/background. The scene observer lives on `MainTabView`, not `ActiveWorkoutView` — the cover leaves the hierarchy when the workout is minimized, so a mini-bar workout would otherwise only be as fresh as the last tick.
- Carries the full in-progress workout: sessionId, workoutDayId, templateName, dayName, startTime, all `SavedExerciseState` + `SavedSetState`. The snapshot deliberately holds only what the DB cannot reproduce — template config (`setScheme`, `repCap`, `restSeconds`, `notes`, `useAddedWeight`) and each set's **GOAL snapshot** (`targetWeight/targetReps/targetRPE`). Without the latter a recovered `SetEntry` falls back to its pending values as the target and persists one it trivially meets, polluting adherence.
- On app launch, `MainTabView.task` checks `WorkoutAutoSave.hasRecoverableState` and shows "Resume Workout?" if a state exists ≤ 4 hours old. Resume is two steps: `restoreFromSavedState` (synchronous, sets + timers) then `hydrateRecoveredContext()` re-fetches previous sets, latest targets and PR baselines with the same calls `startWorkout` makes — the prior session is still the last *completed* one, so they resolve identically.
...
### Program rules: 5/3/1 (`WaveProgression`, `ProgressionRule.wave531`)

The engine is *reactive* (reads the last sessions, decides); 5/3/1 is *scheduled* (TM + wave index determine everything, performance matters only at the cycle boundary). So a wave lift bypasses `calculateTarget` entirely. The seam is an enum switch on `progressionRule` at the four VM sites — `startWorkout`, `completeWorkout`, `hydrateRecoveredContext`, `substituteExercise` — not a protocol; with two rules a union context struct would be mostly optionals.

- **Prescribe** (`startWorkout`): `WaveProgression.prescribe(trainingMax:waveIndex:equipment:)` → three `SetEntry`s with GOAL snapshots. Everything downstream (ring, ledger, adherence, Live Activity, autosave, offline queue) sees ordinary sets. `WaveContext` (TM, wave index, pending review) lives on `ExerciseLogEntry` and in the crash snapshot. RPE is **not** prescribed (5/3/1 doesn't use it). Warm-ups (40/50/60%) go in the COACH card as numbers — never prefilled (hard constraint #1).
- **Record** (`completeWorkout` → `recordWaveSession`): writes a `.wave` journal row carrying the *next* wave's top set, `programWeek = next index`, and this session's + set in `previous_weight/previous_reps`. When the **deload wave** completes, the cycle's three + sets are read back from those rows and `WaveProgression.cycleVerdict` runs; a new `training_maxes` row is written and the journal row carries the verdict (`.increaseWeight` / `.maintain` / `.deload`).
- **Cycle-end verdict** (the one place the engine adds to the book, using the AMRAP e1RM — valid at 1–5 reps, exactly where the engine trusts it): any + set below its minimum → **reset** TM × 0.90, silent (book rule, no override); best e1RM `< TM` → **hold**; best e1RM `> TM ÷ 0.85` (≈12+ on the 5s day) → **recalibrate** to 90% of e1RM; otherwise the standard **bump**, +10 for squat/deadlift (`WaveProgression.cycleBump`: lower-body muscle group *or* "deadlift" in the name, since the library files deadlift under `back`), +5 otherwise. Verdicts are **applied at completion**; the two that depart from the book (`hold`, `recalibrated`) are offered back at the next start with the book's alternative (`trainingMaxReviewBanner`, same shape as Take Deload / Keep Progressing). Detection is stateless: latest TM row `source.deviatesFromBook` and the latest journal row is still the cycle-end row (a completed session writes a `.wave` row after it).
- **Opts out of the reactive nets.** A wave lift is excluded from the performance-deload prompt (its `.deload` is a programmed reset), from the proactive time-based deload apply, and from `keepProgressing`. Nothing autoregulates mid-cycle — the wave is the program.
- **Substitution** drops the slot to `autoregulated` for the session (the wave belongs to the lift; a stand-in has no TM).
- **Logger cues, all string-level:** the + set's ring label reads `SET 3 OF 3 · MAX REPS`, its reps note `5 is the minimum — log what you get`, and the Log button `Log 185 × 5+` until the reps are touched — without them a new lifter logs the minimum and the cycle-end verdict reads "hold" forever. The deload wave carries a one-line "light on purpose" note. Wave lifts' ledger verdicts compare to the *prescription* (HIT / ▲ +n / ▼ MISS), not to the same position last session, and their "Last" lines are hidden — last session was a different wave.
- **BBB seed:** a *hypertrophy* lift with no history on the day but a TM on record prefills every set at 50% of that TM × 10 (`startWorkout`, `bbbSeed`), then ordinary double progression carries it up. When the opposite lift's own day hasn't happened yet (press day carries bench), a **barbell** hypertrophy lift on a day that has a wave lift falls back to an *estimated* TM (best e1RM × 0.9) — scoped that narrowly because 45% of 1RM is far too light as a general first-session default for a 10–15 rep accessory. The catalog's BBB is the **opposite-lift** variant because history and targets are keyed by `(exercise, day)` — the same lift twice on one day would share one prescription (a latent bug for any duplicate exercise on a day, not just BBB).
...
- **Inline** (during the workout): runs in `ActiveWorkoutViewModel.completeSet`. Detects weight PRs (heaviest ever) and rep PRs (most reps at this exact weight). Triggers `prCelebration` modal + heavy haptic. Compares against `PRBaseline` (`ProgressionService.fetchPRBaseline`, derived from `workout_sets` in completed sessions): the heaviest mark plus a best-reps mark **per distinct weight**. Loaded at `startWorkout`, on recovery, and on exercise substitution; advanced in place as sets complete, and a weight PR also seeds the rep mark at that weight so the next set at the same weight is judged against it. A rep PR requires an existing mark at that weight — lifting a lighter weight for the first time is not a record over anything.
- **Session-end**: `ProgressionService.detectPRs()` runs post-completion against DB. Inserts into `personal_records` table. Note `personal_records.reps` means most reps at *any* weight (one row per type — it cannot hold per-weight bests), which is what the Rep Sheet and summary render. It is a different quantity from the inline rep PR and is not used for it.
...
**Programs are a template generator, nothing more.** `ProgramCatalog` (`repIQ/Data/Programs/`) holds hard-coded `ProgramDefinition`s; `ProgramBrowserViewModel.materializeProgram` resolves exercise names to ids and writes ordinary `templates` / `workout_days` / `workout_day_exercises` rows (mode, set scheme, sets, rep cap, rest override, notes). After that a program template is indistinguishable from a custom one — same editor, same engine, same logger. `templates.source_program` is written and never read. The catalog holds only programs the app runs honestly: six hypertrophy splits, Starting Strength / StrongLifts / GZCL, **Wendler's 5/3/1 and 5/3/1 BBB** (on `progressionRule: .wave531` — see "Program rules"), and PHUL / PHAT / Reddit PPL (14 total). The novice programs and every power-day lift use `setScheme: .straight`; GZCL T1 is straight triples via `repCap: 3`, T2 is hypertrophy capped at 10. **nSuns, Texas Method and Conjugate remain cut** (Sep 2026) — their identity is a multi-set percentage ladder, a cross-day dependency, or max-effort/speed work, none of which exists here, and a brand-name program that quietly does something else is a trust break. (5/3/1 was cut in the same pass and restored once it had its own rule.) The editor exposes `Progression: Autoregulated | 5/3/1` on strength lifts, so a custom template can run a wave on any lift; 5/3/1 hides scheme, set count and rep cap. Program `notes` are shown as a NOTE line in the logger, so they must describe what the engine will actually do, not what the source program prescribes.
...
6. User logs sets via `SetLoggerView` — the hero-ring logger, one exercise per screen:
   ...
   - A first-session exercise has no prescription, so its rows start at 0 × 0; once one is logged, the next blank working row takes the same *pending* values (`carryForwardIfBlank`) — pending only, never the GOAL snapshot, so nothing is persisted as a target the set trivially meets. Warm-ups stay blank.
...
- **1.5 (build 15)** — Sep 14 2026 — **Wendler's 5/3/1 and 5/3/1 BBB, on a per-exercise progression rule; Progress tab scoped to one program.** The catalog cut in build 14 was right — the engine couldn't run a percentage cycle — so instead of another `calculateTarget` branch this adds a third axis on `workout_day_exercises`: `progression_rule ∈ {autoregulated, wave_531}` (who decides the numbers, distinct from mode and layout). A wave lift bypasses the engine: `WaveProgression` (pure) prescribes three sets from a **training max** (`training_maxes`, append-only, asked once in the logger via `TrainingMaxCard`, prefilled from best e1RM × 0.9) and the wave index carried in the previously dead `progression_log.mesocycle_week`; mid-cycle rows log a new neutral `.wave` decision so the Progress hero reads the last real verdict instead of 0/4 three weeks out of four. The book's rules are the baseline (percent tables, +5/+10 per cycle, 10% reset on a missed + set); the one addition is at the cycle boundary, where the AMRAP e1RM catches a TM that has drifted above (`hold`) or well below (`recalibrate`) real strength — applied, then offered back with the book's alternative (`trainingMaxReviewBanner`). Nothing autoregulates mid-cycle. Wave lifts are excluded from the performance-deload prompt and the proactive deload apply; substitution drops to autoregulated. BBB ships as the **opposite-lift** variant (bench on press day etc.) because history/targets are keyed `(exercise, day)`; its 5×10 seeds at 50% of that lift's TM and runs double progression from there. Editor: `Progression: Autoregulated | 5/3/1` on strength lifts. Logger: Target card gains a wave-rail row (Wave n of 4 · TM), `5+` AMRAP marking, 40/50/60% warm-ups as COACH numbers (never prefilled). Migration `20260914_progression_rule_and_training_max.sql`.

- **1.5 (build 14)** — Sep 10 2026 — **Inline PR detection and crash recovery made honest.** The "Rep PR — Previous: None" celebration fired on any repeat of a weight: `fetchCurrentPRs` built its `.reps` record as most-reps-at-*any*-weight (`value = reps`) while the inline check looked it up by `value == weight`, so it never matched; and a weight PR never seeded the rep mark at that weight, so the very next set at the same weight "beat" nothing. Replaced the `PersonalRecord` cache with an explicit `PRBaseline` (heaviest + best reps per distinct weight, from `workout_sets`), seeded on weight PRs, and required a prior mark before a rep PR fires. `personal_records`/`detectPRs` untouched — different quantity, consumed by the Rep Sheet. Recovery was worse than it looked: `restoreFromSavedState` rebuilt exercises with no previous sets, no target, default rest, no notes and no PR baseline — the resumed workout behaved like day one — and because `SetEntry` falls back to pending values as its target, every set logged after a resume **persisted a target it trivially met**. The snapshot now carries template config + each set's GOAL snapshot, `hydrateRecoveredContext()` re-fetches history from the DB on resume, substitution loads the new exercise's baseline, and the scene-background save moved from `ActiveWorkoutView` to `MainTabView` so a minimized workout is covered too.
- **1.5 (build 14)** — Sep 10 2026 — **Straight-set strength, and programs that persist what they define.** Pre-built programs were a template generator that silently dropped their own `restSecondsOverride` and `notes` on materialization (columns existed since the initial schema; `addExerciseToDay` never wrote them), and carried a dead `progressionType` + orphan `ProgramEngine` protocol implying a TM/percentage layer that doesn't exist. Both fixed/removed; notes now render as a NOTE line in the logger, and programs can set `repCap`. The real engine gap was that `.strength` *always* ramped, turning every 5×5 / 3×5 program into a one-heavy-set pyramid with an e1RM-sized 5→3 reset. New **`set_scheme`** on `workout_day_exercises` (`20260910_set_scheme.sql`, default `ramped`): `straight` gives every set the prescribed weight, progression is gated by the weakest set, a bump is one increment with reps held at the top (linear progression with rep rebuild on a miss), and adherence grades every set. Editor exposes Ramped/Straight for strength exercises. Recovery snapshots now carry `setScheme` + `repCap` so a recovered workout logs the same decision a normal one would; `takeDeload`'s rebuild now passes `totalSets` (it was defaulting to 4, so a 3-set ramp never reached full weight). **Catalog pass:** cut 5/3/1, 5/3/1 BBB, nSuns, Texas Method and Conjugate (17 → 12 programs); Starting Strength, StrongLifts, Reddit PPL main lifts and PHUL/PHAT power days are now `.straight`; GZCL T1 is straight triples (`repCap: 3`), T2 retagged to hypertrophy with `repCap: 10`; Reddit PPL / SS / SL deadlift is the single top set the programs prescribe; every `notes` string rewritten to describe what the engine does ("add 10 lbs" and "5x3+" AMRAP promises were false under the engine). `ProgramDetailView` / `TemplateDetailView` rows show the capped rep target and the scheme — `(3 reps, straight)` — instead of the raw mode band. Verified end-to-end on the simulator: StrongLifts materializes with rest/notes/scheme, Squat prefills 225 × 5 on all five sets, and after a clean 5×5 the next session prescribes 230 × 5. Also removed the leftover "Share with friends" template toggle and its `is_shared` client plumbing.
...
- **More program rules** (nSuns, Texas Method, Conjugate) — 5/3/1 shipped Sep 2026 as `ProgressionRule.wave531` with `training_maxes`; a new percentage program is a new rule case + a pure prescriber reusing the TM table. Texas Method needs cross-day state the per-exercise seam doesn't have; go template-level only when a program actually requires it. 5/3/1 variants (5s PRO, FSL, leader/anchor, Joker sets) are parameters on the existing rule, each a new promise to keep honest.
- **Progress drill-in TM chart** — `TrainingMaxService.fetchHistory` exists for it; TM line vs e1RM line on `ExerciseProgressView` is the honest progress story for a 5/3/1 run and isn't drawn yet.
- **Same-exercise-twice-on-a-day** — history, targets and `progression_log` are keyed `(exercise, day)`, so two entries collide. Fixing it (key by `workout_day_exercise_id`) is what would allow same-lift BBB.
