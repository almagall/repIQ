...
- **Percentage-based programs** (5/3/1 family, nSuns, Texas Method, Conjugate) — needs a per-user training max per exercise, a week/cycle counter, and a decision vocabulary where a planned deload week lowering targets is not "backing off". The orphan `ProgramEngine` protocol that sketched this was deleted in Sep 2026; start from the engine, not from that.
